...
from ares.api.serializers.v1.resume.link import ResumeLinkSerializer


class ResumeLinkViewSet(viewsets.ModelViewSet):
    """특정 이력서에 대한 링크 CRUD를 위한 ViewSet"""
    ...
    def get_queryset(self):
        """
        요청된 resume_pk에 해당하는 이력서의 링크를 반환합니다.
        ...
        return ResumeLink.objects.filter(resume=resume)

    def perform_create(self, serializer):
        """
        링크 생성 시 URL의 resume_pk를 사용하여 이력서에 자동으로 할당합니다.
        """
        resume = get_object_or_404(
            Resume,
            pk=self.kwargs["resume_pk"],

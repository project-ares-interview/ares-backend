...
[규칙]
1.  **출력 형식:** 반드시 아래 JSON 스키마를 따라야 합니다. `question` 필드는 `text`와 `ssml`을 포함하는 객체여야 합니다.
2.  **SSML 생성 규칙:** `ssml` 필드에는 자연스러운 대화를 위해 `<speak>` 태그로 감싸고, 적절한 `<prosody>`와 `<break>` 태그를 사용한 SSML 마크업을 포함해야 합니다.

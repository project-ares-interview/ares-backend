...
def analysis_worker(sid, audio_buffer, full_transcript, user_gender, total_speaking_time):
    ...
    log.info("[%s] 백그라운드 분석 시작", sid)

    # 1. Voice Analysis
    if audio_buffer and len(audio_buffer) > VOICE_BUFFER_MIN_BYTES:
        try:
            ...
            log.exception("[%s] 음성 분석 워커 오류: %s", sid, e)
    ...
    try:
        log.info("[%s] 텍스트 분석: DB에서 세션 조회 시작", sid)
        session = InterviewSession.objects.select_related('user__profile').get(id=sid)
        log.info("[%s] 텍스트 분석: finalize_interview_session 호출", sid)
        final_report = finalize_interview_session(session)
        log.info("[%s] 텍스트 분석: 최종 리포트 생성 완료", sid)
        ...
        log.exception("[%s] 최종 텍스트 분석 리포트 생성 오류: %s", sid, e)
        async_to_sync(channel_layer.group_send)(group_name, {
            "type": "analysis.event",
            "data": {"event": "error", "data": {"message": f"텍스트 분석 리포트 생성 중 오류 발생: {e}"}}
        })
    
    log.info("[%s] 백그라운드 분석 종료", sid)
    ...
    async def receive_json(self, content):
        ...
        with session_lock:
            ...
            if event == 'toggle_analysis':
                ...
                await self.start_analysis_after_delay(session)
...
class InterviewAudioConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        def stt_callback(text, _duration_sec, event_type):
        ...
        with session_lock:
            ...
            try:
                ...
                log.warning("Azure SDK가 설치되지 않아 STT를 비활성화합니다.")
    ...
    async def check_stt_queue(self):
        while True:
            ...
            try:
                ...
                log.exception("[%s] STT 큐 처리 오류: %s", self.sid, e)
...
class InterviewVideoConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, bytes_data):
        with session_lock:
            session = client_sessions.get(self.sid)
            if not session or not session["metrics"].analyzing:
                return
        ...
        try:
            ...
            log.exception("[%s] 비디오 프레임 처리 오류: %s", self.sid, e)
